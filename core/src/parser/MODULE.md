...
- **Revision-history sections** (FR-34 [D-035]): a paragraph matching `profile.revision_history_label_pattern` arms a consume state; the heading paragraph plus all subsequent non-paragraph blocks (tables and images) are dropped until the next paragraph block (which is by construction the next section's heading or some inter-section text). Multi-page revhist tables — pdfplumber emits each page's slice as its own table block — are consumed as a unit. Counter: `ParseStats.revhist_blocks_dropped`.
