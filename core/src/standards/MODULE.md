...
- `HuggingFaceSource` — class — pub — Download 3GPP spec DOCX files from the GSMA/3GPP HuggingFace dataset.
  ...
  - `_download_file` — method — internal — Stream-download a URL to `dest`.
  - `_list_directory` — method — internal — Return filenames at the HF tree path.
  - `_pick_latest_docx` — staticmethod — internal — Pick the latest version DOCX matching `{compact}-{prefix}*.
...
- `SpecDownloader` — class — pub — Download and cache 3GPP specification documents.
  - `__init__` — constructor — pub
  - `_convert_doc_to_docx` — staticmethod — internal — Convert a.
...
- `spec_to_compact` — function — pub — Remove dots from spec number: '24.
- `spec_to_series` — function — pub — Extract the series from a spec number: '24.
