...
- `XLSXExtractor` (xlsx_extractor.py) — sheet / row / cell extraction with formatting; **planned per FR-1, implementation deferred to development-phase reorg session** (see Deferred)
...
**Deferred**
- `XLSXExtractor` implementation (deferred: planned per FR-1; blocked on dev-phase reorg session — revisit: when reorg lands and module is at `core/src/extraction/`)
