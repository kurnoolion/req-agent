...
- `_strip_markdown_fences` — function — internal — Strip ```json.
- `_validate_profile` — function — internal — Validate (and optionally sanitize) an LLM-emitted profile.
- `_walk_regex_fields` — function — internal — Walk every regex-valued field in the profile.
- `main` — function — pub
...
- `DocumentProfile` — dataclass — pub — Complete document structure profile (TDD 5.
...
- `DocumentProfiler` — class — pub — Derive document structure profiles from representative documents.
  ...
  - `validate_profile` — method — pub — Validate a profile against a document.
<!-- END:STRUCTURE -->
